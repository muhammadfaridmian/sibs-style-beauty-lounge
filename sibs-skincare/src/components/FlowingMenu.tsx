...
// FlowingMenu — React Bits (JS + CSS variant), ported to TypeScript.
// The only addition is an optional onNavigate callback so the links can
// drive the app's curtain transition instead of doing a hard page reload.
...
function MenuItem({
  ...
  const handleClick = (ev: React.MouseEvent<HTMLAnchorElement>) => {
    // When onNavigate is provided, hand the route to the app's curtain
    // transition instead of letting the browser do a full reload.
    if (onNavigate) {
      ev.preventDefault();
      onNavigate(link);
    }
